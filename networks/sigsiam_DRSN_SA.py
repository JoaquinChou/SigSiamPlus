...
class RSBU_SA_BasicBlock(nn.Module):
    def __init__(self, in_channel, out_channel, stride=1, mode=0):
        super(RSBU_SA_BasicBlock, self).__init__()
        self.stride = stride
        self.mode = mode
        ...
        # spatial attention
        self.sa_conv = nn.Conv2d(out_channel, 1, kernel_size=(1, 1), bias=False)
    ...
    def forward(self, x):
        ...
        x = self.conv2(x)
        # spatial attention
        sa_x = x * self.sigmoid(self.sa_conv(x)) 
        ...
        soft_thres_x = torch.mul(torch.sign(sa_x), n_sub_x)
...
# DRSN
class sigsiam_DRSN_SA(nn.Module):
    def __init__(self, in_channel=1, out_channel=64):
        super(sigsiam_DRSN_SA, self).__init__()        
        self.conv1 = nn.Conv2d(in_channel, 4, kernel_size=(3, 1), stride=2, padding=(1, 0), bias=False)
        self.block1 = RSBU_SA_BasicBlock(in_channel=4, out_channel=4, stride=2, mode=1)
        self.block2 = nn.Sequential(RSBU_SA_BasicBlock(in_channel=4, out_channel=4, stride=1, mode=0),
        RSBU_SA_BasicBlock(in_channel=4, out_channel=4, stride=1, mode=0),
        RSBU_SA_BasicBlock(in_channel=4, out_channel=4, stride=1, mode=0))

        self.block3 = RSBU_SA_BasicBlock(in_channel=4, out_channel=16, stride=2, mode=1)
        self.block4 = nn.Sequential(RSBU_SA_BasicBlock(in_channel=16, out_channel=16, stride=1, mode=0),
        RSBU_SA_BasicBlock(in_channel=16, out_channel=16, stride=1, mode=0),
        RSBU_SA_BasicBlock(in_channel=16, out_channel=16, stride=1, mode=0))

        self.block5 = RSBU_SA_BasicBlock(in_channel=16, out_channel=64, stride=2, mode=1)
        self.block6 = nn.Sequential(RSBU_SA_BasicBlock(in_channel=64, out_channel=64, stride=1, mode=0),
        RSBU_SA_BasicBlock(in_channel=64, out_channel=64, stride=1, mode=0),
        RSBU_SA_BasicBlock(in_channel=64, out_channel=64, stride=1, mode=0))

        self.block7 = RSBU_SA_BasicBlock(in_channel=64, out_channel=256, stride=2, mode=1)
        self.block8 = nn.Sequential(RSBU_SA_BasicBlock(in_channel=256, out_channel=256, stride=1, mode=0),
        RSBU_SA_BasicBlock(in_channel=256, out_channel=256, stride=1, mode=0),
        RSBU_SA_BasicBlock(in_channel=256, out_channel=256, stride=1, mode=0))

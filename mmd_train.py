...
def parse_option():
    ...
    # parser.add_argument('--source_train_path', type=str, default='./data_txt/SEU_gearset/SEU_gearset_20_0_train.txt')
    # parser.add_argument('--target_train_path', type=str, default='./data_txt/SEU_gearset/SEU_gearset_30_2_train.txt')
    # parser.add_argument('--test_data_txt_path', type=str, default='./data_txt/SEU_gearset/SEU_gearset_30_2_test.txt')
    # parser.add_argument('--source_train_path', type=str, default='./data_txt/spectra_quest/spectra_quest_Vib_Train_1200_RPM_train.txt')
    # parser.add_argument('--target_train_path', type=str, default='./data_txt/spectra_quest/spectra_quest_Vib_Train_1500_RPM_train.txt')
...
def set_loader(opt):
    ...
    train_sampler = RandomSampler(source_train_dataset, replacement=True, num_samples=2048)
    ...
    target_sampler = RandomSampler(target_train_dataset, replacement=True, num_samples=2048)
    # print("sampler:", [i for i in target_sampler])
...
def train(source_train_loader, target_train_loader, model, bottleneck, classifier, criterion, optimizer_backbone, optimizer_b, optimizer_c, epoch, opt, iter_num, max_iter):
    """one epoch training"""
    model.train()    
    bottleneck.train()
    classifier.train()
    ...
    for (source_singals, source_labels, _), (target_singals, _, _) in zip(source_train_loader, target_train_loader):
        ...
        source_singals = source_singals.float().cuda(non_blocking=True)
        source_labels = source_labels.long().cuda(non_blocking=True)   
        target_singals = target_singals.float().cuda(non_blocking=True)
        bsz = source_labels.shape[0]

        # compute loss
        source_features = model(source_singals)[2]
        target_features = model(target_singals)[2]
        output = classifier(bottleneck(source_features))



        loss = criterion(output, source_labels)
        mmd_loss = mmd_rbf_noaccelerate(source_features, target_features)
        lambd = 2 / (1 + math.exp(-10 * (iter_num) / max_iter)) - 1
        loss += lambd * mmd_loss        
...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
        # train for one epoch
        time1 = time.time()
        loss, acc, iter_num = train(source_train_loader, target_train_loader, model, bottleneck, classifier, criterion,
